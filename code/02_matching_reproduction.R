...
for (depvar in y_spill) {
  m_spill[depvar] <- list(summary(brm(paste(depvar, paste(c(x, contr), collapse = "+"), sep = "~"),
...
for (depvar in y_spill_end) {
  m_spill_end[depvar] <- list(summary(brm(paste(depvar, paste(c(x, contr), collapse = "+"), sep = "~"),
    family = gaussian(link = "identity"), data = wvs_vdem
  )))
}
