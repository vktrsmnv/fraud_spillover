...
To facilitate the substantive interpretation of the estimates, we calculate individuals' probabilities of selecting each of the four answer categories, from \textit{A great deal} to \textit{None at all}, in evaluating institutional trust using draws from expectation of the posterior predictive distributions. 
We then calculate the differences in probabilities for each of the answer-category between those who received fraud information alone and those assigned to another condition--control one or one of the punishment conditions. 
...
We first present the analysis for pooled data from all three countries: figure \ref{fig:main} contains the estimates for the full sample of over 2000 respondents, which varies depending on the number of complete observations per institution, and the main specification from equation \ref{eq:specification}. The dashed line schematically depicts the hypothesized effect, i.e. the point estimates should be somewhat following this trend if the hypothesis is supported by the data. Transparent point ranges include zero in the 89\% highest-density continuous intervals (HDCI). 
...
The Bayes factor calculated for the directional null and complimentary hypotheses (\citealt{_sectionGill2020, Morey2014}), i.e. $BF= \dfrac{P( \text{Data} | \beta_1 \in (0, +\infty))}{ P( \text{Data} |\beta_1 \in (-\infty, 0])}$,  provides further evidence for negative effect of fraud on trust in various political institutions (figure \ref{fig:main} contains the exact Bayes factor values and the relative interpretations based on \citealt{Jeffreys1998}). Overall, we can conclude that the 

We still see some variation across the countries in our sample in this effect: while in Colombia we only observe the effects of fraud for the electoral commission and the president, in Russia and Mexico we see more widespread consequences of fraud information. Besides the obvious explanation--absence of spillover--we should still consider the sample size and, potentially, low quality of responses. 
