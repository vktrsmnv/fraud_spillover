...
As a first step, we only focus on the cases where the respondents in our survey have indicated at least minimal degree of meaningful involvement with the treatment text. We thus remove the few cases where we could not conclude that the respondent has read the text based on the attention check text, e.g. if the summary consisted of a sequence of random symbols. We replicate the main analyses, and figure \ref{fig:main-2} contains the results (tables for the estimates are available upon request). As we can see, in both countries the results from the main analysis hold. 
